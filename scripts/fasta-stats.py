...
def printSequenceStats(fileName, sequenceName, charOccurrenceMap, totalCharCount):
    ...
    """
    print ("\nSequence '{}' from FASTA file '{}' contains {} sequence characters:\n".format(
    ...
    for char in sorted(charOccurrenceMap.keys()):
        if char == "N":
            ...
            coverage = ((totalCharCount - charCount) / totalCharCount) * 100
            print ("\tCoverage is: {0:.2f}%".format(coverage))
            print ("\t{} : {} of {} nucleotides = {:.2f}%".format(char, charCount, totalCharCount, relativeFrequency))
            print ("\n\tNucleotide content:")
    ...
    for char in sorted(charOccurrenceMap.keys()):
        if char != "N":
            charCount = charOccurrenceMap[char]
            relativeFrequency = charCount * 100.0 / totalCharCount
            print ("\t{} : {} = {:.2f}%".format(char, charCount, relativeFrequency))
